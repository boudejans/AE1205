import math


def ex1():
    sum = 0
    for i in range(1000):
        if i % 3 == 0 or i % 5 == 0:
            sum = sum + i
    print("Result: " + str(sum))


def ex2():
    sum = 2
    i = 1
    j = 2
    while j < 4000000:
        k = i + j
        if (k % 2) == 0:
            sum += k
        i = j
        j = k
    print("Result: " + str(sum))


def ex3():
    num = 600851475143
    for i in range(1, 1000000):
        if num % i == 0:
            if IsPrime(i):
                result = i
    print("Result: " + str(result))


def IsPrime(n):
    for i in range(2, n):
        if n % i == 0:
            return False
    return True


def ex4():
    palindrome = 0
    for i in range(100, 999):
        for j in range(100, 999):
            if i * j > palindrome:
                if str(i * j) == str(i * j)[::-1]:
                    palindrome = i * j
    print("Result: " + str(palindrome))


# Find the smallest possible number that is evenly divisible by numbers from 1 to 20
def ex5():
    result = 0
    i = 1
    while result == 0:
        if DivisionCheck(i):
            result = i
        i += 1
    print("Result: " + str(result))


def DivisionCheck(n):
    for i in range(1, 21):
        if n % i != 0:
            return False
    return True


# Calculate the difference between the square of sum and sum of squares of the first 100 natural numbers
def ex6():
    sumOfSquares = 0
    sum = 0
    for i in range(0, 101):
        sumOfSquares += i ** 2
        sum += i
    difference = sum ** 2 - sumOfSquares
    print("Result: " + str(difference))


# Find the 10,001 st prime number
def ex7():
    prime_index = 0
    prime_number = 2
    number = 2
    while prime_index < 10001:
        if IsPrime(number):
            prime_number = number
            prime_index += 1
        number += 1
    print("Result: " + str(prime_number))


# Find the largest product of thirteen digits in the 1000-digit number
def ex8():
    number = ""
    largestProduct = 0
    for i in range(0, 1000 - 13):
        section = number[i:i + 13]
        product = 1
        for chr in section:
            product = product * int(chr)
        if product > largestProduct:
            largestProduct = product
    print("Result: " + str(largestProduct))


# Find the pythagorean triplet a + b + c = 1000
def ex9():
    a = 0
    b = 0
    c = 0
    result = 0
    for i in range(0, 400):
        for j in range(0, 400):
            a = i
            b = j
            c = math.sqrt(a ** 2 + b ** 2)
            if a + b + c == 1000:
                result = a * b * c
    print("Result: " + str(result))


# Find the sum of all the prime numbers up until 2,000,000
def ex10():
    sum = 2
    for i in range(2, 2000000):
        if IsPrimeV2(i):
            sum += i
        if i % 1000 == 0:
            print(i)

    print("Result: " + str(sum))


def IsPrimeV2(n):
    for i in range(2, math.ceil(math.sqrt(n)) + 1):
        if n % i == 0:
            return False
    return True


def ex54():
    p1_wins = 0
    with open("Poker.txt") as f:
        lines = f.readlines()
        for line in lines:
            p1_hands = line.split(" ")[0:5]
            p2_hands = line.split(" ")[5:10]
            p2_hands[-1] = p2_hands[-1].strip()
            p1_suits = [0] * 4
            p1_numbers = [0] * 13
            p2_suits = [0] * 4
            p2_numbers = [0] * 13
            p1_score = 0
            p2_score = 0
            p1_highestCard = 0
            p2_highestCard = 0
            p1_highestCardPair = 0
            p2_highestCardPair = 0
            p1_valueNumbers = []
            p2_valueNumbers = []
            p1_flush = 0
            p2_flush = 0
            # Go through each of the five hands
            for card in p1_hands:
                match card[1]:
                    case "C":
                        p1_suits[0] += 1
                    case "S":
                        p1_suits[1] += 1
                    case "H":
                        p1_suits[2] += 1
                    case "D":
                        p1_suits[3] += 1
                value = 0
                if not card[0].isdigit():
                    match card[0]:
                        case "T":
                            value = 10
                        case "J":
                            value = 11
                        case "Q":
                            value = 12
                        case "K":
                            value = 13
                        case "A":
                            value = 14
                else:
                    value = int(card[0])
                p1_valueNumbers.append(value - 1)
                p1_numbers[value - 2] += 1
                if value > p1_highestCard:
                    p1_highestCard = value  # High card
            for suit in p1_suits:
                if suit == 5:
                    # p1_score = 18  # Flush
                    p1_flush = 1
            if (max(p1_valueNumbers) - min(p1_valueNumbers) + 1) == 5:
                if p1_flush == 1:
                    if p1_highestCard == 14:
                        p1_score = 22  # Royal Flush
                    else:
                        p1_score = 21  # Straight Flush
                        p1_highestCardPair = max(p1_valueNumbers)
                else:
                    p1_score = 17  # Straight
                    p1_highestCardPair = max(p1_valueNumbers)
            else:
                for number in p1_numbers:
                    if number == 2:
                        if p1_score == 14:
                            p1_score = 15  # Two pair
                            if p1_numbers.index(number) > p1_highestCardPair:
                                p1_highestCardPair = p1_numbers.index(number)
                        elif p1_score == 16:
                            p1_score = 19  # Full house
                        elif p1_score == 0:
                            p1_score = 14  # One pair
                            p1_highestCardPair = p1_numbers.index(number)
                    if number == 3:
                        if p1_score == 14 or p1_score == 15:
                            p1_score = 19  # Full house
                            p1_highestCardPair = p1_numbers.index(number)
                        elif p1_score < 16:
                            p1_score = 16  # Three of a kind
                            p1_highestCardPair = p1_numbers.index(number)
                    if number == 4:
                        if p1_score < 20:
                            p1_score = 20  # Four of a kind
                            p1_highestCardPair = p1_numbers.index(number)
            if p1_score < 18 and p1_flush == 1:
                p1_score = 18
            for card in p2_hands:
                match card[1]:
                    case "C":
                        p2_suits[0] += 1
                    case "S":
                        p2_suits[1] += 1
                    case "H":
                        p2_suits[2] += 1
                    case "D":
                        p2_suits[3] += 1
                value = 0
                if not card[0].isdigit():
                    match card[0]:
                        case "T":
                            value = 10
                        case "J":
                            value = 11
                        case "Q":
                            value = 12
                        case "K":
                            value = 13
                        case "A":
                            value = 14
                else:
                    value = int(card[0])
                if value > p2_highestCard:
                    p2_highestCard = value  # High card
                p2_valueNumbers.append(value - 1)
                p2_numbers[value - 2] += 1
            for suit in p2_suits:
                if suit == 5:
                    # p2_score = 18  # Flush
                    p2_flush = 1
            if (max(p2_numbers) - min(p2_numbers) + 1) == 5:
                if p2_flush == 1:
                    if p2_highestCard == 14:
                        p2_score = 22  # Royal Flush
                    else:
                        p2_score = 21  # Straight Flush
                        p2_highestCardPair = max(p2_valueNumbers)
                else:
                    p2_score = 17  # Straight
                    p2_highestCardPair = max(p2_valueNumbers)
            else:
                for number in p2_numbers:
                    if number == 2:
                        if p2_score == 14:
                            p2_score = 15  # Two pair
                            if p2_numbers.index(number) > p2_highestCardPair:
                                p2_highestCardPair = p2_numbers.index(number)
                        elif p2_score == 16:
                            p2_score = 19  # Full house
                        elif p2_score == 0:
                            p2_score = 14  # One pair
                            p2_highestCardPair = p2_numbers.index(number)
                    if number == 3:
                        if p2_score == 14 or p2_score == 15:
                            p2_score = 19  # Full house
                            p2_highestCardPair = p2_numbers.index(number)
                        elif p2_score < 16:
                            p2_score = 16  # Three of a kind
                            p2_highestCardPair = p2_numbers.index(number)
                    if number == 4:
                        if p2_score < 20:
                            p2_score = 20  # Four of a kind
                            p2_highestCardPair = p2_numbers.index(number)
            if p2_score < 18 and p2_flush == 1:
                p2_score = 18
            if p1_score > p2_score:
                p1_wins += 1
            elif p1_score == p2_score:  # If both players have the same win type
                if p1_highestCardPair > p2_highestCardPair:
                    p1_wins += 1
                elif p1_highestCardPair == p2_highestCardPair:
                    p1_valueNumbers.sort()
                    p2_valueNumbers.sort()
                    print(str(p1_valueNumbers[-1]) + " - " + str(p1_valueNumbers[-2]) + " - " + str(
                        p1_valueNumbers[-3]) + " - " + str(p1_valueNumbers[-4]) + " - " + str(p1_valueNumbers[-5]))
                    print(str(p2_valueNumbers[-1]) + " - " + str(p2_valueNumbers[-2]) + " - " + str(
                        p2_valueNumbers[-3]) + " - " + str(p2_valueNumbers[-4]) + " - " + str(p2_valueNumbers[-5]))
                    if p1_valueNumbers[-1] == p2_valueNumbers[-1]:
                        # Check which card is higher of the first card that is different
                        if p1_valueNumbers[-2] == p2_valueNumbers[-2]:
                            if p1_valueNumbers[-3] == p2_valueNumbers[-3]:
                                if p1_valueNumbers[-4] == p2_valueNumbers[-4]:
                                    if p1_valueNumbers[-5] > p2_valueNumbers[-5]:
                                        p1_wins += 1
                                elif p1_valueNumbers[-4] > p2_valueNumbers[-4]:
                                    p1_wins += 1
                            elif p1_valueNumbers[-3] > p2_valueNumbers[-3]:
                                p1_wins += 1
                        elif p1_valueNumbers[-2] > p2_valueNumbers[-2]:
                            p1_wins += 1
                    elif p1_valueNumbers[-1] > p2_valueNumbers[-1]:
                       p1_wins += 1
            print(str(lines.index(line)) + "| " + str(p1_score) + " - " + str(p2_score) + " || " + str(p1_highestCard) + " - " + str(p2_highestCard) + " || " + str(p1_wins) + " || " + str(p1_highestCardPair) + " - " + str(p2_highestCardPair))
    print("Result: " + str(p1_wins))


ex54()
